...
wilid.spp.lu2 <- read_csv("./data/wilid_spp_lu_v2.csv") %>% 
  select(c(species, wilid_full)) %>% 
  distinct()

## change ref system to start with lu
lu.wilid.spp.v2 <- read_csv("./data/wilid_spp_lu_v2.csv") %>% 
  select(c(species, wilid_full)) %>% 
  distinct()
...
This reads in and munges data from "2017_Raw_Production_20180423.xlsx", accessed from the team drive after entry by LM+
```{r}
...
p17.raw %>%
  tabyl(site, plot) %>% 
  gt() %>% 
  tab_header(title = "Count of observations in 2017_Raw_Production_20180423.xlsx")
...
p17.raw <- p17.raw %>% 
  ...
  mutate(yr = "2017") %>% 
  mutate(yr = as.character(yr)) %>% 
  mutate(source = "2017_Raw_Production_20180423.xlsx")
...
```{r}
p17.ht <- p17.raw %>% 
  select(c(yr, season, site, site2, plot, species, wilid, pl_ht_cm, source)) %>% 
  mutate(wilid_full = paste0(site,"-",plot,"-",wilid))
...
p17.ht %>% 
  ...
  datatable(caption = "2017_Raw_Production_20180423.xlsx")

p17.ht %>%
  group_by(wilid_full) %>%
  tally() %>% 
  datatable(caption = "Distinct wilid: 2017_Raw_Production_20180423.xlsx")

```

There are problematic wilid evident (e.g., wilid = ?, NA for wilid)
...
p17.ht %>% 
  ggTile_yr_season_site2() +
  labs(caption = "2017_Raw_Production_20180423.xlsx")

```

## 2017 Utilization
This reads in and munges data from *2017_Raw_Utilization.xlsx*, accessed from the team drive after entry by LM+
...
u17.raw <- u17.raw %>% 
  ...
  mutate(plot = if_else(is.na(plot),"obs",plot)) %>%
  mutate(site2 = paste0(site, "-", plot)) %>% 
  mutate(source = "2017_Raw_Utilization.xlsx")

u17.raw <- u17.raw %>%
  filter(!is.na(pl_ht_cm)) %>%  
  select(c(yr, site, site2, plot, wilid, season, yr, pl_ht_cm, source)) %>% 
  mutate(pl_ht_cm = as.numeric(pl_ht_cm)) # 
...
u17.ht %>% 
  filter(is.na(wilid)) %>% 
  gt() %>% 
  tab_header(title = "Missing wilid from 2017_Raw_Utilization.xlsx")
...
u17.ht %>% 
  ggTile_yr_season_site2() +
  labs(caption = "2017_Raw_Utilization.xlsx")
...
# this plot reveals a mis-entered ht of 3119. I checked the raw file it should be 319

```

```{r}
# plot reveals a mis-entered ht of 3119. I checked the raw file it should be 319. Change
u17.ht <- u17.ht %>% 
  mutate(pl_ht_cm = case_when(pl_ht_cm == 3119 ~ 319,
         TRUE ~ pl_ht_cm))
...
p18 <- files2018 %>% 
  ...
  clean_names() %>% 
  mutate(source = str_remove(file_name, "./data/raw/production/Raw_2018_Production/"))

## create a "source" field for tracking
...
p18.exp %>% 
  ...
  tabyl(source) %>% 
  gt() %>% 
  tab_header(title = "2018 production data", subtitle = "Raw input files: exp")

p18.obs %>% 
  filter(!is.na(sl)) %>% 
  tabyl(source) %>% 
  gt() %>% 
  tab_header(title = "2018 production data", subtitle = "Raw input files: obs")

## each of the sites has data entered into a separate, single-tabbed excel file

```


```{r}
### work with the combined obj and exp import
p18 <- p18 %>% 
  janitor::clean_names() %>% 
  mutate(yr = 2018) %>%
  mutate(yr = as.character(yr)) %>% 
...
p18sel <- p18 %>% 
  select(yr, season, spp, site, plot, wilid, pl_ht_cm, source) %>%
  mutate(pl_ht_cm = as.numeric(pl_ht_cm)) %>% 
  distinct()
...
p18sel %>%
  visdat::vis_miss() +
  labs(caption = "Source: 2018 production data(many source xlsx files)")
...
p18sel %>% 
  filter(is.na(wilid)) %>% 
  gt() %>% 
  tab_header(title = "2018 production files: missing 'wilid'")
...
p18sel %>% 
  tabyl(source) %>% 
  gt() %>% 
  tab_header(title = "2018 production data", subtitle = "Raw input files (numerous)")
