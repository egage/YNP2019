...
# !!!! possible issie with some wilid
# some cleaning 
# p16.raw <- p16.raw %>% 
...
p16.raw %>% 
  distinct(site, wilid) %>% 
  arrange(site) %>% 
  datatable(rownames = FALSE, caption = "Distinct wilid in raw 2016 production excel file:2016_Raw_Production_20180421.xlsx")
...
## ha. 21 different files -- some of which may have multi tabs
## split out the obs
...
## different structure
p18.exp %>% 
  filter(!is.na(stid_6) | !is.na(stid_50)) %>% 
  distinct(file_name, sheet_name)

p18.exp %>% 
  filter(!is.na(datez_year_month_day)) %>% 
  distinct(file_name, sheet_name)
# ./data/raw/production/Raw_2018_Production/e~ eb1_dx
...
```{r}
## 
# eliminate  columns without data. I don't know if date is important, but it's a mess, so just eliminating
p18 <- p18 %>% 
  select(!c(x127,date,date_year_month_day))
...
## corect error with plot = "3" to obs
u18u19.ht <- u18u19.ht %>%
