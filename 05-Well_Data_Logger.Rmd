...
The code above scrapes data from raw logger files discovered while searching for data in the different shared dropbox/drive folders and DK hard drives. 

DCC were processed to yield relative water table elevations (by DK), while newer files need correction for stickup/hang depth. These data are presumed to exist somewhere and are needed to evaluate possible errors in barometric corrections used from the sole barotroll (Tower) I found for the post-DCC period. As already indicated, these data are raw and need connection to field measurements (e.g., hang height, well stickup). Combined data retain file_name as an attribute to aid in the necessary detective work using field notes.
All csv files were uncorrected. Data were corrected using barometric pressure data from the Tower barotroll, the only baro data I’ve found for the >2016 time period. I did come across a logger at Mammoth, but data don't extend past the DCC data. Loggers vary in their start and logging interval, so to synchronize, data were reduced to a daily step. 
