...
dcc.dly.mean %>% # names()
  ...
  geom_line(aes(color=yr)) +
  labs(x = "Day of year", y = "Water table elevation (m)") +
  facet_wrap(~site_logger, scale = 'free_y', ncol = 2) +
  theme_minimal()
# ggsave("waterlevel_loggers_exp_alt2.png", width = 8.5, height = 6)
...
logger.all %>%
  ...
  labs(x="", y="Head (cm)", caption = "logger.all+DCC")
...
I've come across raw files with records predating 2016 (the end of DCC data), but have excluded them, assuming the DCC captured them.

The code scrapes data from raw files exported from the loggers and discovered while searching for data in the different shared dropbox/drive folders and DK hard drives. 

DCC were processed to yield relative water table elevations (by DK), while newer files need correction for stickup/hang depth. These data are presumed to exist somewhere(?!) and are needed to evaluate possible errors in barometric corrections used from the sole barotroll (Tower) I found for the post-DCC period. As already indicated, these data are raw and need connection to whatever field measurements were made (e.g., hang height, well stickup). Combined data retains the file_name as an attribute to aid in the necessary detective work (probably Lewis who has access to whatever field notes exist and has site knowledge I lack).
All csv files were uncorrected. Data were corrected using barometric pressure data from the Tower barotroll. This is the only baro data I’ve found for the >2016 time period. Loggers vary in their start and logging interval, so to synchronize, data were reduced to a daily step. 
