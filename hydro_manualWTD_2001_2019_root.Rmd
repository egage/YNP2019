...
This document describes data import, wrangling, and merging of manual well measurements collected over the project lifetime. Three primary data sets are processed: 
...
```{r, echo=FALSE}
run.date <- format(Sys.time(), '%Y%m%d')
```
...
## tidy version for plotting
lu_site_beav.tidy <- lu_site_beav %>% 
  pivot_longer(
    cols = beav19:beav15,
    names_to = "bv",
    values_to = "bv_value"
...
## add yr
lu_site_beav.tidy <- lu_site_beav.tidy %>% 
  mutate(yr = case_when(bv == "beav19" ~ 2019,
                        bv == "beav18" ~ 2018,
                        bv == "beav17" ~ 2017,
                        bv == "beav16" ~ 2016,
                        bv == "beav15" ~ 2015,
...
```{r, eval=FALSE,, echo=FALSE, fig.height=8, fig.width=6}
lu_site_beav.tidy %>% 
  filter(plot == "obs") %>%
  group_by(yr, site2, plot, bv_value) %>% 
  tally() %>% 
  mutate(n = as.character(n)) %>% 
  ggplot(aes(yr, n)) +
  geom_point(aes(color = bv_value, shape = bv_value), size=3) +
  ...
  labs(x= "", color ="", shape = "", caption = "Beaver occupancy from './data/lu_site_site2_beav.csv'")
...
sel.01.19 %>%
  write_csv("./data/processed/manual_well_data_raw_20201201.csv")
...
```{r, eval= FALSE}
sel.01.19.summary %>%
  write_csv("./output/processed_data/manual_WTD_site_mean_20201201.csv")
